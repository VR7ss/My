document.addEventListener('DOMContentLoaded', function() {
   ...
   // New function to fetch exploit status
       ...
       try {
           ...
           exploits.forEach(exploit => {
               ...
               if (appCard) {
                   ...
                   const versionTag = appCard.querySelector('.version-tag[data-version-target="true"]');

                   if (appBadge) {
                       if (exploit.detected) {
                           appBadge.textContent = 'غير شغال 🔴';
                           appBadge.style.backgroundColor = '#dc3545'; // Red
                       } else if (exploit.updateStatus) {
                           appBadge.textContent = 'شغال 🟢';
                           appBadge.style.backgroundColor = '#28a745'; // Green
                       } else {
                           appBadge.textContent = 'يحتاج تحديث 🟡';
                           appBadge.style.backgroundColor = '#ffc107'; // Yellow
                       }
                   }
           ...
           // Optionally, update badges to an error state or default
           ...
           document.querySelectorAll('.version-tag[data-version-target="true"]').forEach(tag => {
               if (tag.textContent === 'جاري التحميل...') {
                   tag.textContent = 'خطأ';
               }
           });
       }
